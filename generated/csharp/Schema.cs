...
namespace Generated {
    ...
    public class Codec {
        ...
        public static byte[] EncodePlayer(Player obj) {
            using (var ms = new MemoryStream()) {
                using (var w = new BinaryWriter(ms)) {
                    ...
                    WriteInt(w, obj.name.Length); w.Write(Encoding.UTF8.GetBytes(obj.name));
                    
                    
                    
                    WriteInt(w, obj.level);
                    
                    
                    
                    WriteInt(w, obj.inventory.Length);
        ...
        public static Player DecodePlayer(byte[] data) {
            ...
            using (var ms = new MemoryStream()) {
                zip.CopyTo(ms);
                ms.Position = 0;
                return DecodePlayerInternal(new BinaryReader(ms));
            }
            
        }
        
        private static Player DecodePlayerInternal(BinaryReader r) {
            // 1. Check Version
            int vLen = ReadInt(r);
